...
public class OnlyPositives {
    ...
    public static void main(String[] args) {
		...
		try{
			...
			System.out.println("Please enter Integer format");
    	}
		scanner.close();
    }
}
...
public class OnlyPositives {
	private static final Logger logger = Logger.getLogger(OnlyPositives.class.getName());
	...
	public static void main(String[] args) {
		...
		try {
			logger.info("OnlyPositives program started");
			...
			while (true) {
				try {
					...
					if (inputNumber > 0) {
						...
						logger.info("Calculated: " + inputNumber + "^2 = " + result);
					} else if (inputNumber < 0) {
						System.out.println("Unsuitable number");
						logger.warning("Negative number rejected: " + inputNumber);
						continue;
					} else {
						logger.info("Zero entered, exiting program");
						break;
					}
				} catch (NumberFormatException e) {
					System.out.println("Please enter Integer format");
					logger.log(Level.WARNING, "Invalid number format entered", e);
					// Continue loop
				}
			}
		} catch (Exception e) {
